...
from dataset import load_data_norm, get_preprocess_fn, rawTorqueDataset, get_collate_fn, determinedTorqueDataset, onDiskDataset, cartisianTorqueDataset, mixedDataset
from engine import train_one_epoch, evaluate, calc_norm, train_one_epoch_mix
...
def train(config, network, criterion, metric, optimizer, scheduler, train_loader, train_preprocess_fn, test_loader, test_preprocess_fn, data_norm,
    ...
    for epoch in range(start_ep, config.TRAIN.EPOCHS):
        if config.DATASET.TRAIN.get('MODE', 'raw') == 'mix':
            train_stats, global_step = train_one_epoch_mix(network, criterion, train_loader, train_preprocess_fn, data_norm, 
...
if __name__ == '__main__':
    ...
    if config.DATASET.TRAIN.get('MODE', 'raw') == 'adb':
        ...
        train_dataset = onDiskDataset(config.DATASET.TRAIN, split='train')
    elif config.DATASET.TRAIN.get('MODE', 'raw') == 'mix':
        train_dataset = mixedDataset(config.DATASET.TRAIN, split='train')
    else:
    ...
    if config.DATASET.TEST.get('MODE', 'raw') == 'adb':
        ...
        test_dataset = onDiskDataset(config.DATASET.TEST, split='test')
    elif config.DATASET.TEST.get('MODE', 'raw') == 'mix':
        test_dataset = mixedDataset(config.DATASET.TEST, split='test')
    else:
